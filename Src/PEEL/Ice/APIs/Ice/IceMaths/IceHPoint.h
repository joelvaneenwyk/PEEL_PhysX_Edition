...
#define ICEHPOINT_H
	...
	class ICEMATHS_API HPoint : public Point
	{
		...
		inline_	HPoint&		Set(const float f[4])							{ x  = f[X_];	y  = f[Y_];	z  = f[Z_];	w  = f[W_];		return *this;	}
		...
		//! Add a vector
		inline_	HPoint&		Add(const float f[4])							{ x += f[X_];	y += f[Y_];	z += f[Z_];	w += f[W_];		return *this;	}

		//! Subtract a vector
		inline_	HPoint&		Sub(float _x, float _y, float _z, float _w )	{ x -= _x;		y -= _y;	z -= _z;	w -= _w;		return *this;	}
		//! Subtract a vector
		inline_	HPoint&		Sub(const float f[4])							{ x -= f[X_];	y -= f[Y_];	z -= f[Z_];	w -= f[W_];		return *this;	}
